...
// sbt-assembly creates a JAR of your project with all of its dependencies(:=Maven)
// 1. Add sbt-assembly as a dependency in project/plugins.sbt
// 2. Update otherwise it will complain; add following lines

import sbtassembly.AssemblyKeys.{assemblyJarName, assemblyMergeStrategy}
import sbtassembly.{MergeStrategy, PathList}

lazy val projectName = "scala-clustering"
...
lazy val commonSettings = Seq(
  ...
  homepage := Some(url("http://youngjaehwang.tech"))
)
...
lazy val dependenciesSettings = Seq(
  // sbt resolver is the configuration for repository that contains jars and their
  // dependencies; below is a resolver definition for a repository jira and so on
  resolvers ++= Seq(
    "Atlassian Releases" at "https://maven.atlassian.com/public/",   // Jira
    // sbt plugin for publishing bintray packages
    // https://github.com/sbt/sbt-bintray
    "scalaz-bintray" at "https://dl.bintray.com/scalaz/releases",    // Sonatype
    Resolver.sonatypeRepo("snapshots"),
    Classpaths.typesafeReleases,
    Classpaths.sbtPluginReleases
  ),
  libraryDependencies ++= Seq(
    "ch.qos.logback" % "logback-classic" % "1.1.2", // logger, can be ignored in play framework
    "com.databricks" %% "spark-csv" % "1.5.0",
    "com.github.fommil.netlib" % "all" % "1.1.2",   // Linear Algebra
